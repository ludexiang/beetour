...
<body>
	<header>
		<div style="text-align: center;color: red;font-size: 20px;">banner图片修改</div>
	</header>
	<div style="padding-top: 20px; padding-left:200px;">
	<form:form modelAttribute="banner" method="post" action="update">
		<input type="hidden" name="id" value="${banner.id}"/>				
		<div>
			<label>banner图片介绍:</label><br>
			<textarea id="content" rows="5" cols="55" name="content">${banner.content}</textarea>
		</div><br/>
		<div>
			...
			<div id="container_pic">
				...
				</div><label id='picUrl'>${banner.pic}</label><br>
			<input id='pic' name="pic" type="hidden" value="${banner.pic}"/><br/><br/>			
		</div><br/>
		<div>
			<label>是否展示 </label>
			<select name="show">
				<option value="0" <c:if test= "${banner.show == 0}">selected</c:if> >否</option>
				<option value="1" <c:if test= "${banner.show == 1}">selected</c:if> >是</option>
			</select> 
		</div><br/>
